...
const ChooseBudgetModal = ({
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 font-semibold text-gray2 text-xs sm:text-sm">
        You can select as much as you’d like but you must select at least 5
        categories
      </p>

      <div className="mt-7 flex gap-3 sm:gap-6 flex-wrap">
        {selectedOption.map((budget, index) => (
          <button
            onClick={() => setSelectedBudget(budget)}
            key={index}
            className={`py-2 sm:py-4 px-3 sm:px-5 text-xs sm:text-sm flex items-center gap-1 sm:gap-2 font-semibold border rounded-full transition-colors duration-300  ${
      ...
      <div className="flex justify-center">
        <button
          ...
          className="mt-8 w-full py-3 sm:py-5 font-semibold text-white rounded-lg bg-primary transition-colors duration-300 disabled:bg-gray3"
        >
          Done
        </button>
      </div>
